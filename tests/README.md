...
| File | Description |
|------|-------------|
| `test_code_diagrams_generator.py` | Code-flow generator internals: target resolution/output paths, deterministic timestamps and visible flow-content freshness digests, idempotent whole-tree source-marker reconciliation, orphan pruning, formatter failure propagation, and bounded Playwright screenshot scaling. |
| `test_diagram_artifact_contract.py` | Committed diagram contract: exact target/artifact/index/timestamp/marker symmetry, required high-value flow subset, Pillow verification and nonzero dimensions for all 84 PNGs, and the fixed six-stack/two-aggregate infrastructure catalogue. |
