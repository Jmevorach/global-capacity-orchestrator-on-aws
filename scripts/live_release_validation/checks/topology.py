...
_ALB_TARGET_CONVERGENCE_TIMEOUT_SECONDS = 5 * 60
...
def _alb_https_target_evidence(
    ...
    *,
    ...
    """Prove the controller realized HTTPS target groups with healthy pod targets.

    Desired-state manifests are not sufficient release evidence: this reads the
    exact internal ALB and its attached target groups from ELBv2. A healthy
    target proves the ALB completed its configured HTTPS ``/healthz`` probe
    against a workload listener.
    """
    ...
    for target_group in sorted(
        ...
        deadline = time.monotonic() + _ALB_TARGET_CONVERGENCE_TIMEOUT_SECONDS
        while True:
            ...
            if remaining <= 0:
                raise RuntimeError(
                    ...
                    f"within {_ALB_TARGET_CONVERGENCE_TIMEOUT_SECONDS} seconds; states={states}"
                )
